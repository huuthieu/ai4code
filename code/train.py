...
parser.add_argument('--experiment_path', type=str, default='/media/aiteam/storage/AI_DATA/data/kaggle_data/google_ai_code/exp')

args = parser.parse_args()
if not os.path.exists("./outputs"):
    os.mkdir("./outputs")
# data_dir = Path('..//input/')
data_dir = Path("/media/aiteam/storage/AI_DATA/data/kaggle_data/google_ai_code/contest_data")
...
def train(model, train_loader, val_loader, epochs):
    ...
    print("Num iteration:", len(train_loader)/4)
    for e in range(epochs):
        ...
        for idx, data in tqdm(enumerate(tbar),leave = False):
            inputs, target = read_data(data)
...
def train_with_mlflow(run_name, model, train_loader, val_loader, epochs):
    np.random.seed(0)
    # Creating optimizer and lr schedulers
    param_optimizer = list(model.named_parameters())
    no_decay = ['bias', 'LayerNorm.bias', 'LayerNorm.weight']
    ...
    with mlflow.start_run(run_name=run_name) as run:
        for e in range(epochs):
            ...
            for idx, data in tqdm(enumerate(tbar),leave = False):
                ...
                fig = view_loss(plot_loss)
                fig.savefig('train_loss.png')
                mlflow.log_artifact(fig, "train_loss.png")
            ...
            torch.save(model.state_dict(), "./outputs/model.bin")
            log_model(model, 'torch_model')
            fig = view_score(plot_score)
            fig.savefig('val_score.png')
            mlflow.log_artifact(fig, "val_score.png")
...
model = model.cuda()
# model.load_state_dict(torch.load('outputs/ver1/model.bin'))
